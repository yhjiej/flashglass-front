...
const CarouselDots = React.forwardRef<HTMLElement, React.ComponentProps<typeof Button>>(({ className, ...props }, ref) => {
	const { scrollSnaps, selectedIndex, onDotButtonClick } = useCarousel()

	return (
		<nav ref={ref} className={cn('flex flex-nowrap items-center justify-center gap-3', className)} {...props}>
			{scrollSnaps?.length > 0 &&
				scrollSnaps.map((_, index) => (
					<button
						key={index}
						aria-label={`Go to slide ${index + 1}`}
						className={cn('w-4 h-4 rounded-full bg-gray-700 opacity-40 hover:opacity-100 transition-all duration-300', selectedIndex === index && 'opacity-100')}
